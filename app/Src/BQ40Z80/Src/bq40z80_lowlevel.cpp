...
HAL_StatusTypeDef Driver::readWord(uint8_t cmd, uint16_t& data) {
    ...
    SEGGER_RTT_printf(0, "BQ40Z80: Reading word from cmd=0x%02X (addr: write=0x%02X, read=0x%02X)\n", 
    ...
    if (status != HAL_OK) {
        SEGGER_RTT_printf(0, "BQ40Z80: Failed to send command 0x%02X (status: %d)\n", cmd, status);
        return status;
    }
    ...
    if (status != HAL_OK) {
        SEGGER_RTT_printf(0, "BQ40Z80: Failed to read data for command 0x%02X (status: %d)\n", cmd, status);
        return status;
    }
    
    // Debug: Print buffer after read
    SEGGER_RTT_printf(0, "BQ40Z80: Buffer after read: 0x%02X 0x%02X\n", buffer[0], buffer[1]);
    ...
    SEGGER_RTT_printf(0, "BQ40Z80: Read word 0x%04X from cmd=0x%02X\n", data, cmd);
...
HAL_StatusTypeDef Driver::writeWord(uint8_t cmd, uint16_t data) {
    ...
    if (status != HAL_OK) {
        SEGGER_RTT_printf(0, "BQ40Z80: Failed to write word to command 0x%02X (status: %d)\n", cmd, status);
        return status;
    }
...
HAL_StatusTypeDef Driver::readBlock(uint8_t cmd, std::vector<uint8_t>& data) {
    ...
    if (status != HAL_OK) {
        SEGGER_RTT_printf(0, "BQ40Z80: Failed to send block read command 0x%02X (status: %d)\n", cmd, status);
        return status;
    }
    ...
    if (status != HAL_OK) {
        SEGGER_RTT_printf(0, "BQ40Z80: Failed to read block data (status: %d)\n", status);
        return status;
    }
    ...
    SEGGER_RTT_printf(0, "BQ40Z80: Read %d bytes from block\n", length);
...
HAL_StatusTypeDef Driver::writeBlock(uint8_t cmd, const std::vector<uint8_t>& data) {
    ...
    SEGGER_RTT_printf(0, "BQ40Z80: Writing block cmd=0x%02X, len=%d, total=%d\n", cmd, data.size(), totalLength);
    ...
    if (status != HAL_OK) {
        SEGGER_RTT_printf(0, "BQ40Z80: Block write failed (status: %d)\n", status);
        return status;
    }
    
    SEGGER_RTT_printf(0, "BQ40Z80: Block write completed successfully\n");
    return HAL_OK;
}
...
HAL_StatusTypeDef Driver::manufacturerCommand(uint16_t command) {
    ...
    if (status != HAL_OK) {
        SEGGER_RTT_printf(0, "BQ40Z80: Failed to send ManufacturerAccess command (status: %d)\\n", status);
    }
...
HAL_StatusTypeDef Driver::manufacturerBlockAccessRead(uint16_t command, uint16_t& data) {
    SEGGER_RTT_printf(0, "BQ40Z80: Using ManufacturerBlockAccess method for command 0x%04X\n", command);
    
    // Step 1: Send MAC command via ManufacturerBlockAccess (0x44)
    HAL_StatusTypeDef status = manufacturerBlockWrite(command);
    if (status != HAL_OK) {
        SEGGER_RTT_printf(0, "BQ40Z80: Failed to send ManufacturerBlockAccess command (status: %d)\n", status);
        return status;
    }
    
    // Step 2: Wait for BQ40Z80 to process the MAC command
    SEGGER_RTT_printf(0, "BQ40Z80: Waiting for MAC command processing...\n");
    ...
    if (status == HAL_OK && response.size() >= 4) {
        ...
        if (echo_command == command) {
            ...
            SEGGER_RTT_printf(0, "BQ40Z80: ManufacturerBlockAccess success - command 0x%04X = 0x%04X\n", 
                             command, data);
            return HAL_OK;
        } else {
            SEGGER_RTT_printf(0, "BQ40Z80: MAC command mismatch - sent 0x%04X, got echo 0x%04X\n", 
                             command, echo_command);
            return HAL_ERROR;
        }
    } else {
        SEGGER_RTT_printf(0, "BQ40Z80: ManufacturerBlockAccess read failed (status: %d) or insufficient data (%d bytes)\n", 
...
HAL_StatusTypeDef Driver::manufacturerRead(uint16_t command, uint16_t& data) {
    ...
    SEGGER_RTT_printf(0, "BQ40Z80: Reading ManufacturerData BEFORE sending command 0x%04X\n", command);
    ...
    if (readBlock(0x23, preCmdData) == HAL_OK && preCmdData.size() >= 2) {
        ...
        SEGGER_RTT_printf(0, "BQ40Z80: Pre-command data: 0x%04X, size: %d bytes\n", preData, preCmdData.size());
    }
    
    // Try different approaches to clear the register  
    SEGGER_RTT_printf(0, "BQ40Z80: Attempting to clear ManufacturerData register\n");
    ...
    if (readBlock(0x23, clearCheckData) == HAL_OK && clearCheckData.size() >= 2) {
        ...
        SEGGER_RTT_printf(0, "BQ40Z80: After clear attempt: 0x%04X\n", clearData);
    }
    
    // Send manufacturer command to ManufacturerAccess (0x00)
    SEGGER_RTT_printf(0, "BQ40Z80: Sending MAC command 0x%04X\n", command);
    HAL_StatusTypeDef status = manufacturerCommand(command);
    if (status != HAL_OK) {
        SEGGER_RTT_printf(0, "BQ40Z80: Failed to send manufacturer command 0x%04X\n", command);
        return status;
    }
    ...
    SEGGER_RTT_printf(0, "BQ40Z80: Waiting %dms for MAC command 0x%04X processing\n", processingDelay, command);
    ...
    for (int i = 0; i < retries; i++) {
        ...
        if (status == HAL_OK && blockData.size() >= 2) {
            ...
            SEGGER_RTT_printf(0, "BQ40Z80: Raw block data (%d bytes): ", blockData.size());
            for (size_t j = 0; j < blockData.size() && j < 16; j++) {
                SEGGER_RTT_printf(0, "0x%02X ", blockData[j]);
            }
            SEGGER_RTT_printf(0, "\n");
            ...
            if (isAscii && blockData.size() > 4) {
                ...
                SEGGER_RTT_printf(0, "BQ40Z80: Possible ASCII data: '%s'\n", asciiData.c_str());
            }
            
            // For now, still use the first 2 bytes as little-endian data
            data = blockData[0] | (blockData[1] << 8);
            SEGGER_RTT_printf(0, "BQ40Z80: Attempt %d - Read manufacturer data 0x%04X for command 0x%04X\n", 
                             i+1, data, command);
            
            // Debug: Check if this is ASCII characters (0x6261 = 'ab' in ASCII)
            if (data == 0x6261) {
                char ascii_chars[3] = {blockData[0], blockData[1], 0};
                SEGGER_RTT_printf(0, "BQ40Z80: Constant 0x6261 = '%c%c' in ASCII - possible protocol error\n", 
            ...
            if (i < retries - 1) {
                SEGGER_RTT_printf(0, "BQ40Z80: Data unchanged, waiting additional 50ms\n");
                HAL_Delay_MS(50);
            }
        } else if (status == HAL_OK) {
            SEGGER_RTT_printf(0, "BQ40Z80: Insufficient data in ManufacturerData block (size: %d)\n", blockData.size());
            status = HAL_ERROR;
            break;
        } else {
            SEGGER_RTT_printf(0, "BQ40Z80: Failed to read ManufacturerData for command 0x%04X (attempt %d)\n", command, i+1);
            if (i < retries - 1) {
    ...
    if (status != HAL_OK) {
        SEGGER_RTT_printf(0, "BQ40Z80: All attempts failed for MAC command 0x%04X\n", command);
    }
...
HAL_StatusTypeDef Driver::manufacturerBlockWrite(uint16_t mac_command) {
    ...
    SEGGER_RTT_printf(0, "BQ40Z80: Sending MAC command 0x%04X via ManufacturerBlockAccess (0x44) as %02X %02X\n", 
...
HAL_StatusTypeDef Driver::manufacturerBlockRead(std::vector<uint8_t>& data) {
    SEGGER_RTT_printf(0, "BQ40Z80: Reading response from ManufacturerBlockAccess (0x44)\n");
    ...
    if (status == HAL_OK && data.size() >= 4) {
        ...
        SEGGER_RTT_printf(0, "BQ40Z80: ManufacturerBlockAccess response (%d bytes): ", data.size());
        ...
        SEGGER_RTT_printf(0, "BQ40Z80: MAC command in response: 0x%04X\n", mac_command);
        
        // Extract actual data (bytes 2-3+ in little-endian)
        if (data.size() >= 4) {
            uint16_t result = data[2] | (data[3] << 8);
            SEGGER_RTT_printf(0, "BQ40Z80: MAC result: 0x%04X\n", result);
        }
        
        return HAL_OK;
    } else if (status == HAL_OK) {
        SEGGER_RTT_printf(0, "BQ40Z80: ManufacturerBlockAccess returned insufficient data (%d bytes)\n", data.size());
        return HAL_ERROR;
    } else {
        SEGGER_RTT_printf(0, "BQ40Z80: Failed to read ManufacturerBlockAccess (status: %d)\n", status);
        return status;
    }
}
